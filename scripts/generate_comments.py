...
2. **잘한 단원 칭찬** (강점으로 표시된 단원) — 구체적 단원명
3. **부족한 단원 보충 계획** (약점으로 표시된 단원) — 구체적 단원명
4. **재시 이력이 있으면** 극복 과정 긍정적으로 표현 (이미 본 재시는 OK, 앞으로의 재시 예고는 절대 금지)
5. **난이도별 학습 패턴** 코멘트 — 예:
...
[★ 점수 비공개 규칙 — 매우 중요 ★]
- **학생의 학습지 점수(숫자)를 코멘트에 절대 명시하지 마세요.**
  · 금지: "85점", "92점을 기록", "60점에 그쳤습니다", "70점 미만" 등
  · 금지: "평균 80점대", "고득점", "저득점" 같이 구체적 점수 영역을 암시하는 표현
- 대신 **두루뭉실한 정성 표현**으로 학습 결과를 전달하세요:
  · 강점 단원 → "우수한 성취를 보였습니다" / "안정적으로 잘 풀어냈습니다" /
                "이해도가 돋보였습니다" / "탄탄한 실력을 보여주었습니다"
  · 약점 단원 → "조금 부족한 부분이 있었습니다" / "다소 어려움을 겪었습니다" /
                "보완이 필요한 부분이 보였습니다" / "추가 연습이 필요해 보입니다"
  · 전반 평가 → "전반적으로 양호합니다" / "꾸준한 성장이 엿보입니다" /
                "성실하게 임하고 있습니다"
- 재시 이력(이미 본 재시)은 "재시를 통해 회복했습니다" 식으로 점수 없이 결과만 언급
- 학부모가 구체적 점수는 가통문 표(주간평가/선행평가)에서 직접 확인하므로,
  AI 코멘트는 **정성적 해석과 지도 방향**에만 집중

[★ 재시 예고 금지 규칙 — 매우 중요 ★]
- **앞으로의 재시·보충시험을 예고하는 문구를 절대 작성하지 마세요.**
  · 금지: "오는 달에 재시를 치를 예정입니다"
  · 금지: "다음 달 재시를 통해 보완하겠습니다"
  · 금지: "재시험으로 점검할 예정입니다", "재평가가 진행됩니다" 등 미래의 재시 언급 일체
- 이유: 선생님이 평가로 간주하지 않은 학습지에까지 자동으로 '재시' 코멘트가 달리는
  부작용이 있어, 미래 재시 예고는 모두 제거합니다.
- 약점 단원 보완은 재시 표현 없이 "꾸준히 보완해 나가겠습니다" /
  "추가 학습으로 다지겠습니다" / "반복 연습으로 채워가겠습니다" 식으로만 표현하세요.
...
def format_student_for_prompt(student: dict) -> str:
    ...
    # 재시 예정 학습지는 의도적으로 모델에 전달하지 않음.
    # 미래의 재시 예고가 자동으로 코멘트에 박히는 부작용을 방지하기 위함.
    # (학원 운영상 일부 학습지는 평가 대상이 아니라 재시도 진행하지 않음)

    # 학습 분석 (난이도, 학습량, 강점/약점)
    # ※ 점수는 모델에게 전달하지 않음 — 정성 라벨로만 표현 유도
    ...
    if analytics.get("total_count"):
        ...
        if by_diff:
            ...
            for d in sorted(by_diff.keys(), key=lambda x: int(x)):
                info = by_diff[d]
                # 점수는 빼고 학습 분량(건수/문항)만 노출
                lines.append(
                    f"- 난이도 {d} ({info['label']}): {info['count']}건, {info['total_problems']}문항"
                )

        if analytics.get("strengths"):
            lines.append(f"\n[잘한 단원 — '우수한 성취' 같은 정성 표현으로 칭찬]")
            for s in analytics["strengths"]:
                lines.append(f"- {s['단원']}")

        if analytics.get("weaknesses"):
            lines.append(f"\n[부족한 단원 — '조금 부족한' / '보완 필요' 같은 부드러운 표현으로 언급]")
            for w in analytics["weaknesses"]:
                lines.append(f"- {w['단원']}")

    # 주간평가 (가통문 표에 점수가 표시되므로 모델에는 점수 없이 단원/재시 여부만 전달)
    if student.get("weekly_evals"):
        lines.append(f"\n[주간 평가 진행 단원 — 점수는 가통문 표에 표기됨, 코멘트엔 점수 쓰지 말 것]")
        for e in student["weekly_evals"]:
            재시 = f" (재시{e['재시차수']}차)" if e.get("재시차수", 0) > 0 else ""
            주차 = f"{e.get('주차', '?')}주차"
            lines.append(f"- {주차} {e['단원명']}{재시}")

    # 선행평가
    if student.get("unit_evals"):
        lines.append(f"\n[선행 평가 진행 단원 — 점수는 가통문 표에 표기됨, 코멘트엔 점수 쓰지 말 것]")
        for e in student["unit_evals"]:
            재시 = f" (재시{e['재시차수']}차)" if e.get("재시차수", 0) > 0 else ""
            lines.append(f"- {e['단원명']}{재시}")

    # 재시 예정 학습지에서도 점수는 위 [재시 예정] 섹션에서 빼야 하므로 재구성 안내
    return "\n".join(lines)
